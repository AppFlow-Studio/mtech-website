...
async function page() {
  const dollarStores = await sanityFetch({
    query: defineQuery(`*[_type == "BUSINESS_TYPES" && business_type_link == "/dollar-stores"]`),
    ...options,
  });
  if (!dollarStores) {
  ...
  return (
    <BusinessTypePage businessType={dollarStores.data[0]} />
  );
}
