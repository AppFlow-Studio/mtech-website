...
async function page() {
  const beautySalon = await sanityFetch({
    query: defineQuery(`*[_type == "BUSINESS_TYPES" && business_type_link == "/beauty-salon"]`),
    ...options,
  });
  if (!beautySalon) {
    return (
  ...
  return (
    <BusinessTypePage businessType={beautySalon.data[0]} />
  )
}
